...
local function GetConfigFolder(configName)
    return "ASCEND/" .. configName
end

local function EnsureConfigFolder()
    if isfolder and not isfolder("ASCEND") then
        makefolder("ASCEND")
    end
end
...
local function GetAvailableConfigs()
    ...
    if isfolder and listfiles then
        EnsureConfigFolder()
        local files = listfiles("ASCEND")
        for _, file in ipairs(files) do
            local name = file:match("ASCEND/(.+)%.json$") or file:match("ASCEND\\(.+)%.json$")
            if name then
