...
if _GE._LORD_LOADED then return end
_GE._LORD_LOADED = true
...
local s = {
    lord = {Width = 690, Height = 446},
    SliderMin = {Width = 500, Height = 300},
    SliderMax = {Width = 1200, Height = 800},
...
local icons = {
    Main = "rbxassetid://98889368832741",
    ...
    ConfigSave = "rbxassetid://10723356507",
}
...
local function GetConfigFolder(configName)
    return "ASCEND/Lord/" .. configName
end

local function EnsureConfigFolder()
    if isfolder and makefolder then
        if not isfolder("ASCEND") then makefolder("ASCEND") end
        if not isfolder("ASCEND/Lord") then makefolder("ASCEND/Lord") end
    end
end
...
local function GetAvailableConfigs()
    ...
    if isfolder and listfiles then
        EnsureConfigFolder()
        local files = listfiles("ASCEND/Lord")
...
local AscendBlur = {}
AscendBlur.__index = AscendBlur

function AscendBlur.new(object)
...
function AscendBlur:_CreateDepthOfField()
    local existingDOF = lg:FindFirstChild("AscendBlur")
    if existingDOF then
        existingDOF:Destroy()
    end
    local existingBlur = lg:FindFirstChild("AscendBlurEffect")
...
function AscendBlur:_CreateFolder()
    local existingFolder = workspace.CurrentCamera:FindFirstChild("AscendBlur")
...
function AscendBlur:_CreateRoot()
...
function AscendBlur:_CreateFrame()
...
function AscendBlur:_Render(distance)
...
function AscendBlur:_Initialize()
...
function AscendBlur:SetEnabled(enabled)
...
function AscendBlur:Destroy()
    if self._folder then
        self._folder:Destroy()
    end
    local dof = lg:FindFirstChild("AscendBlur")
    if dof then
        dof:Destroy()
    end
    local blur = lg:FindFirstChild("AscendBlurEffect")
    if blur then
        blur:Destroy()
    end
end
...
function Library.new(title, configFolder)
    ...
    self.title = title or "Ascend"
    self.configFolder = configFolder or title or "Ascend"
    self.sections = {}
    self.currentTab = nil
    self.minimized = false
    self._AscendBlur = nil
    self._keybinds = {}
    self._toggleKey = Enum.KeyCode.RightControl
    self._visible = true
    self._originalHeight = s.lord.Height
    self._originalWidth = s.lord.Width
    self._minSize = Vector2.new(s.SliderMin.Width, s.SliderMin.Height)
    self._maxSize = Vector2.new(s.SliderMax.Width, s.SliderMax.Height)
    self._configElements = {}
    self._autoSave = false
    self._currentConfig = "default"
    self:_CreateMainlord()
...
function Library:Notify(config)
    ...
    local icon = config.Icon or icons.Notification
...
function Library:Toggle()
    ...
    if self._AscendBlur then
        self._AscendBlur:SetEnabled(self._visible)
    end
...
function Library:_CreateMainlord()
    ...
    self.container = CreateInstance("Frame", {
        ...
        Position = UDim2.new(0.5, -s.lord.Width / 2, 0.5, -s.lord.Height / 2),
        BorderSizePixel = 0,
        Size = UDim2.new(0, s.lord.Width, 0, s.lord.Height),
    ...
    minimizeIcon.MouseEnter:Connect(function()
        CreateTween(minimizeIcon, {ImageColor3 = c.Text}, animationspeed.Fast)
    end)

    minimizeIcon.MouseLeave:Connect(function()
        CreateTween(minimizeIcon, {ImageColor3 = c.TextDark}, animationspeed.Fast)
    end)
    ...
    local targetParent = CrGui
    self.screenGui.Parent = targetParent
    self._AscendBlur = AscendBlur.new(self.container)
end

function Library:_CreatelordControls()
...
function Library:_ToggleMinimize()
    ...
    if self.minimized then
        ...
        local minimizedWidth = math.max(self.titleLabel.AbsoluteSize.X + 60, 120)
        if self._AscendBlur then
            self._AscendBlur:SetEnabled(false)
        end
        ...
        if self._AscendBlur then
            self._AscendBlur:SetEnabled(true)
        end
